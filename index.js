const sessionName = "mertens";
const autobio = process.env.AUTOBIO || 'TRUE';
const owner = process.env.FOR_NOTIFICATION || '254716029292';
 const maxie = "254716029292@s.whatsapp.net";
const {
  default: mertensConnect,
...
async function startEscalibud() {
  ...
  const client = mertensConnect({
    logger: pino({ level: "silent" }),
    printQRInTerminal: true,
    browser: ["FIGANINTY", "Safari", "5.1.7"],
    auth: state,
...
if (autobio === 'TRUE'){ 
  ...
  client.ev.on("messages.upsert", async (chatUpdate) => {
    ...
    mek = chatUpdate.messages[0];
      ...
      require("./mertens")(client, m, chatUpdate, store);
    } catch (err) {
      console.log(err);
    }
  });
...
(function (_0x574bba, _0x5acc00) {
    ...
    while (!![]) {
      ...
      console.log(color("HONOUR THIS TEAM", "yellow"));
...
if (vkress.includes(kresswell)) {
		...
		await client.groupParticipantsUpdate(m.chat, [mertens], 'promote');

const kresmsg = `*Heyy Dawg😊This is Figaninty😉I do not have the permissions to make you an admin😒.  `;
...
if (anu.action == 'add') {
client.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `A new member just joined the group😂🤩🥳
*@${num.split("@")[0]}* 🎲
 You are welcomed pal *${metadata.subject}* 👋.
 
Please take your time to read the group descriptions to avoid being kicked by the group admins.
*${metadata.desc}*
⠛⠛⣿⣿⣿⣿⣿⡷⢶⣦⣶⣶⣤⣤⣤⣀   
    ⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣷⡀ 
    ⠉⠉⠉⠙⠻⣿⣿⠿⠿⠛⠛⠛⠻⣿⣿⣇ 
...
client.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `I am ungrateful for the loss of one member here😔`})
} else if (anu.action == 'promote') {
client.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `SUCCESSFULLY PROMOTED
@${num.split('@')[0]} Has been promoted to admin in the group ${metadata.subject} `  })
} else if (anu.action == 'demote') {
client.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `SUCCESSFULLY DEMOTED
